#!/usr/bin/env python3
"""
Verbessertes Test-Skript für MaStR Lead Generator
Behebt CSRF-Token-Probleme und testet die Anwendung umfassend
"""
...
class ApplicationTester:
    ...
    def test_admin_login(self):
        """Testet Admin-Login mit verbesserter CSRF-Behandlung"""
        ...
        headers = {'Referer': urljoin(BASE_URL, '/accounts/login/')}
        ...
        if response.status_code in [200, 302]:
            ...
            self.log(f"❌ Admin-Login fehlgeschlagen - Status: {response.status_code}", "ERROR")
            return False
    ...
    def test_user_registration(self):
        """Testet Benutzerregistrierung mit verbesserter CSRF-Behandlung"""
        ...
        # Referer-Header setzen
        headers = {'Referer': urljoin(BASE_URL, '/accounts/register/')}
        
        response = self.session.post(
            urljoin(BASE_URL, "/accounts/register/"), 
            data=register_data, 
            headers=headers,
            timeout=10
        )
        ...
        if response.status_code in [200, 302]:
            ...
            self.log(f"❌ Benutzerregistrierung fehlgeschlagen - Status: {response.status_code}", "ERROR")
            return False
...
def main():
    """Hauptfunktion"""
    print("🧪 MaStR Lead Generator - Verbesserter Anwendungstest")
    print("=" * 60)
